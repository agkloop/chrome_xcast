...
| Sandbox | Deny by default. It can use the network, read the system trust store, `stat` its own install folder (macOS's certificate verifier requires that), and write only to its own `data/` folder. It cannot read or list your files or start other programs, with one exception: `/usr/bin/caffeinate`, which keeps the computer from idle sleep while the TV reads from it. It runs in the same sandbox, the helper can signal only its own children, and at worst it keeps the computer awake. |
